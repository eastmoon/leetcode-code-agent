...
1. 自公開資訊取得題目，生成題目檔案於 [topic](./topic) 目錄
...
一段程式碼要如何解讀並描述成功能文件，這部分則屬於人工智慧的研究領域，其最重要理論源於 **軟體逆向工程**、**程式碼摘要** 相關原則，而其認知流程如下：

+ 參考前述的 **憲章與指南** 設計，完成 **程式碼摘要** 憲章與 **程式語言摘要** 指南。
+ 對目標程式碼檔案，用 **程式語言摘要** 指南解讀，並基於 **虛擬碼編碼** 憲章為風格，將程式碼轉為虛擬碼。
+ 對目標虛擬碼檔案，用 **程式碼摘要** 憲章解讀，並產生議題樣板生成摘要文件。

如上程序，實際程式碼認知範例流程如下：

1. 基於題目與指定程式語言，生成不嚴謹程式碼，其中程式碼特徵如下：
  + 不使用編碼指南
  + 用兩種程式碼壞味道 ( Code Smell ) 風格撰寫
2. 基於不嚴謹程式碼，執行程式碼摘要，並生成摘要文件於，[bad](./bad) 目錄中，例如摘要問題 1 的 C++ 檔案在 [0001_Two-Sum](./bad/0001_Two-Sum) 目錄
  + ```cs-pseudo.md``` 為程式碼轉成的虛擬碼
  + ```cs-topic.md``` 為虛擬碼轉成摘要文件
3. 基於摘要文件與需求文件比對，得到 ```topic-different.md``` 檔案，此檔案描述需求 ( 功能預期 ) 與摘要 ( 功能實作 ) 的實際差異。
