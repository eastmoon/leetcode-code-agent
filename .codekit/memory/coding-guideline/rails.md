...
- 禁止保留被註解掉的程式碼；框架產生器（Generator）產生的預設樣板註解若無實質內容須清除。
...
- View 中重複的畫面片段須抽取為 Partial（`_partial.html.erb`）並以 `render` 呼叫；重複的查詢邏輯須抽取為 Model Scope（例如 `scope :active, -> { where(active: true) }`）。
...
- 外部串接（如金流、郵件、推播）須以 Adapter 或 Gateway 物件封裝，並透過建構子注入（例如 `def initialize(gateway: DefaultGateway.new)`），禁止在 Service 內部直接以硬編碼方式實例化具體外部客戶端。
